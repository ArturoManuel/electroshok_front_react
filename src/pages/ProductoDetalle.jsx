    ...
    useEffect(() => {
        ...
        if (nombreProducto) {
            const prod = productos.find(
                (p) => p.nombre.trim().toLowerCase() === decodeURIComponent(nombreProducto).trim().toLowerCase()
            );
            ...
            <section className="product-detail-section">
                <div className="product-detail">
                    ...
                    <div className="product-detail-info">
                        ...
                        <p className="product-stock">Stock: <span className="stock-value">15</span> unidades</p>
                        <p className="product-detail-price">S/ {producto.precio.toFixed(2)}</p>
                        <div className="product-detail-quantity">
                            <button className="quantity-btn" onClick={() => setCantidad(c => Math.max(1, c - 1))}>-</button>
                            <input className="quantity-input" type="number" value={cantidad} readOnly />
                            <button className="quantity-btn" onClick={() => setCantidad(c => Math.min(15, c + 1))}>+</button>
                        </div>
